...
impl ParseBytes for DecimalBytes {
    ...
    fn parse_4_bytes(s: &[u8]) -> u32 { // no need to benchmark this, to be used later
    ...
    fn parse_8_bytes(b : &[u8]) -> u32 {
      let (upper_digits, lower_digits) = b.split_at(4);
      Self::parse_4_bytes(upper_digits) * 10000 + Self::parse_4_bytes(lower_digits)
    }
  }